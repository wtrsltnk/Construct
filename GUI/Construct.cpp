...
Construct::Construct()
{
	this->userinput = new UserInput(Construct::GetWindowsKeyMap());
...
void Construct::OnInitialize()
{
	...
	tVidProps p = { this->dwWidth, this->dwHeight, 32, true };
	
	this->SetVideoProps(p);

	this->world.LoadWorld("c:\\de_village.bsp");
...
void Construct::OnIdle()
{
	...
	::glTexCoord2f(1.0f, 0.0f); ::glVertex3f(6.0f, 0.0f, 0.0f);
	::glEnd();
...
LRESULT Construct::ObjectProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
		...
		case WM_KEYDOWN:
		{
			...
			{
				static bool b = false;
				static tVidProps p = { this->dwWidth, this->dwHeight, 32, false };
